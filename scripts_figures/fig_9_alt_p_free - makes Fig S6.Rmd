...
# Load the parameters for the fits that estimate p

load("../fitting/DataObjects/flow_sim_frac_pfree.RData")

parms_frac <- map(flow_sim_frac_pfree, ~c("beta" = .[["par"]][["beta"]],
                                "Theta" = .[["par"]][["Theta"]],
                                "p" = .[["par"]][["p"]]))
...
parms_frac_bootstrap <- map(flow_sim_frac_pfree, ~.$bootstrap[,c("Theta", "beta", "p")])
...
pdf("../figures/fig_S6.pdf", width=plotwidth, height=plotwidth*0.9)#/golden.ratio)
print(fig)
dev.off()
pdf("../../Manuscript/PaperFigures/Figure-S6.pdf", width=plotwidth, height=plotwidth*0.9)#/golden.ratio)
print(fig)
dev.off()
