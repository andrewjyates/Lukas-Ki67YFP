...
start_time <- Sys.time()
simultaneous_fit_frac_pfree <- map(simultaneous_flow_data_fitting, ~fit(datas = .,
    odes = flow_frac,
...
shapiro <- map(simultaneous_fit_frac_pfree, ~shapiro.test(.[["residuals"]]))
flow_sim_frac_pfree <- simultaneous_fit_frac_pfree 
save(flow_sim_frac_pfree, file = "DataObjects/flow_sim_frac_pfree.Rdata")
simultaneous_flow_frac_pfree_parms <-map(flow_sim_frac_pfree, ~.[["par"]])

# approx confidence intervals
interval=list()
for(cell in c("CD4", "CD8")){
fisher_info = solve(simultaneous_fit_frac_pfree[[cell]]$hessian)
pars=simultaneous_fit_frac_pfree[[cell]]$par
...
save(interval, file = "DataObjects/flow_sim_frac_pfree_HessianEstimates.Rdata")
...
model_data <- map(simultaneous_flow_frac_pfree_parms, ~ run(tmax=80, tstep=0.1,
    odes = flow_frac,
    table = TRUE,
    parms = .,
    ymax = 1,
...
flow_model_simultaneous_pfree_table <- all_data_model
write_rds(flow_model_simultaneous_pfree_table, file = "../scripts_figures/flow_model_simultaneous_pfree_table")
```



```{r}
save.image("DataObjects/flow_model_together_pfree.RData")
```
