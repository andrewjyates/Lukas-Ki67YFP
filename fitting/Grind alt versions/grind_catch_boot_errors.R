...
fit <- function(datas=data, state=s, parms=p, odes=model, free=NULL, who=NULL, differ=NULL, fixed=NULL, tmin=0, tmax=NULL, ymin=NULL, ymax=NULL, log="", xlab="Time", ylab="Density", bootstrap=0, bootstrap.residuals=FALSE, cores=NULL, show=NULL, fun=NULL, costfun=cost, initial=FALSE, add=FALSE, timeplot=TRUE, legend=TRUE, main=NULL, sub=NULL, pchMap=NULL, ...) {
  dots <- list(...)
  if (!is.null(dots)) {
  ...
  if ( bootstrap.residuals ) { ## bootstrap by sampling the residuals x-xhat
    ...
    #imat <- sapply(seq(bootstrap), function(i) {
      ...
      ifit <- try(do.call('modFit',c(list(f=costfun,p=found,datas=samples,odes=odes,state=state,parms=parms,free=free,differ=differ,fixed=fixed,fun=fun,initial=initial,isVar=isVar),dots_fit)))
      ifelse(inherits(ifit, "try-error"), rep(NA, length(found)), ifit$par)
    })
...
cost <- function(datas, odes, state, parms, guess, free, differ, fixed, fun, initial, isVar, ...) {
  ...
  for (iset in seq(nsets)) {
    ...
    totcost <- do.call('modCost',c(list(model=nsol,obs=data,cost=totcost, weight="none"),dots_fit)) 
  }
  return (totcost)
}
