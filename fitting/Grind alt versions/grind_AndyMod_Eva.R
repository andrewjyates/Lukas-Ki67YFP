...
library(parallel)   # for parallel bootstrapping
...
fit <- function(datas=data, state=s, parms=p, odes=model, free=NULL, sigma_scale = NULL, who=NULL, differ=NULL, fixed=NULL, tmin=0, tmax=NULL, ymin=NULL, ymax=NULL, log="", xlab="Time", ylab="Density", bootstrap=0, bootstrap.residuals=FALSE, cores=NULL, show=NULL, fun=NULL, costfun=cost, initial=FALSE, add=FALSE, timeplot=TRUE, legend=TRUE, main=NULL, sub=NULL, pchMap=NULL, ...) {
  
  dots <- list(...)
  
  # sigma_scale_length <- ncol(datas[1])-1
  # print(sigma_scale_length)
  # print(paste("ncol_data = ", ncol(datas[[1]])))
  ...
  #if(length(sigma_scale) != sigma_scale_length)stop("Scale list has to be the same number as indicators")
  ...
  f <- do.call('modFit',c(list(f=costfun,p=guess,datas=datas,odes=odes,state=state,parms=parms,free=free,differ=differ,fixed=fixed,fun=fun,initial=initial,isVar=isVar, sigma_scale = sigma_scale),dots_fit))
  found <- f$par
  ...
  if (bootstrap == 0) return(f) ## no BS
  if ( is.null(cores) ) {
      cores <- max(1, detectCores()-1)
  }
  cat(sprintf("bootstrapping with %s cores\n", cores))
  clus <- makeCluster(cores) 
  clusterEvalQ(clus, library("FME"))
  clusterExport(clus, c("args_run", "args_fit", "args_plot", "run", "args_steady", "methods_run", "methods_fit"), envir=environment())
  if ( bootstrap.residuals ) { ## bootstrap by sampling the residuals x-xhat
    imat <- parSapply(clus, seq(bootstrap), function(i) {
    #imat <- sapply(seq(bootstrap), function(i) {
      samples <- lapply(seq(nsets),function(j) {
        residuals <- f$residuals
        varnames <- unique(names(residuals))
        residuals <- sapply(varnames, function(x) unname(residuals[names(residuals) == x]))

        residual.samples <- residuals[sample(nrow(residuals),replace=TRUE),]

        bs.data <- exp(fun(datas[[j]][varnames]) - residuals + residual.samples) ## replace residuals with randomized residuals
        bs.data[["time"]] = datas[[j]][["time"]]
        bs.data
      })
  ...
  } else { ## bootstrap by sampling with replacement pairs (t, x)
    imat <- parSapply(clus, seq(bootstrap), function(i) {
      samples <- lapply(seq(nsets),function(j){
...
cost <- function(datas, odes, state, parms, guess, free, differ, fixed, fun, initial, isVar, sigma_scale, ...) {
  dots <- list(...)
  if (!is.null(dots)) {
    dots_run <- dots[names(dots) %in% args_run]
...
# Setting scaleVarValue individually per column, time column index 0    
#givng sigma_scale as a list input to fit to loop through in parameter estimation 
#print(scale_sigma)

#if(iset == 1){scaleVarValue = 1}
#if(iset ==2){scaleVarValue = 0.25}

  #cat(iset, "\n")
  #cat(ncol(data), "\n")
  #cat(ncol(nsol), "\n\n")
    
  # Andy code for breaking the dataframe into sections, one for each variable, and adding the sigma_scale
  # value as the "err" argument
  # commented out for now. So for now this file will behave identically to Rob's original grind.R code.
  # for(i in 2:ncol(data)){
  #   thiscol.data=cbind(data[,c(1, i)], std=sigma_scale[i-1])
  #   thiscol.nsol=nsol[,c(1,i)]
  #   totcost <- do.call('modCost',c(list(model=thiscol.nsol,obs=thiscol.data,cost=totcost, err="std"),dots_fit)) 
  # }
  #browser()
